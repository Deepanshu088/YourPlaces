...
const getUsers = async(req, res, next)=>{
    ...
    try{
        ...
        return next(new HttpError('Couldnt fetch users. Try again!!',500));
    }
...
const signup = async (req, res, next)=>{
    ...
    let token;
    try{
        token = jwt.sign({userId: newUser._id, email: email}, process.env.JWT_SECRET, { expiresIn: '1h'});
    }catch(e){
        console.log(e);
...
const login = async (req, res, next)=>{
    ...
    let token;
    try{
        token = jwt.sign({userId: existingUser[0]._id, email: email}, process.env.JWT_SECRET, { expiresIn: '1h'});
    }catch(e){
        console.log(e);
